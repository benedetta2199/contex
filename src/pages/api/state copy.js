...
export const currentInitialization = () => {
  ...
  /*Resetta tutti i valori a quelli del primo utilizzo*/
  const resetAll = () => {
    initializeZone();
  ...
  const initializeGeoPoI = async () => {
    const tables = ['teatri', 'chiese', 'scuole', 'impianti_sportivi', 'aree_verdi', 'musei', 'negozi', 'biblio', 'fermate', 'parcheggi', 'ospedali', 'stazioni', 'piste_ciclabili'];
    const feature = new Map(poi);
    for (let i = 0; i < tables.length; i++) {
      const name = Array.from(feature.keys())[i];
      const url = `http://localhost:5000/api/data?table=${tables[i]}`;
      try {
        const response = await fetch(url);
        if (!response.ok) throw new Error('Network response was not ok ' + response.statusText);
        const data = await response.json();
        feature.set(name, { ...feature.get(name), geoJSON: convertToGeoJSON(data) });
      } catch (error) {
  ...
  const initializeHouse = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/casa?raggio=${raggio}&questionario=${getRispQuestionario()}`);
      if (!response.ok) throw new Error('Network response was not ok ' + response.statusText);
      const data = await response.json();
      setHouse(data);
  ...
  const initializeHouseBici = async () => {
    setFilterHouseT([]);
    setLoading(prevState => ({ ...prevState, caseT: true }));
    const url = `http://localhost:5000/api/bike?tempo=${time}&questionario=${getRispQuestionario()}`;
    try {
      const response = await fetch(url);
      if (!response.ok) throw new Error('Network response was not ok ' + response.statusText);
      const data = await response.json();
      setHouseT(data);
      setFilterHouseT(data.slice(0, nhouseT + 1));
    } catch (error) {
      console.error('Error:', error);
    } finally {
  ...
  const initializeSuggestArea = async () => {
    setLoading(prevState => ({ ...prevState, cluster: true }));
    const url = `http://localhost:5000/api/suggest_locations?questionario=${getRispQuestionario()}`;
    try {
      const response = await fetch(url);
      if (!response.ok) throw new Error('Network response was not ok ' + response.statusText);
      const data = await response.json();
    ...
    } finally {setLoading(prevState => ({ ...prevState, cluster: false }));
    }
  };

  /**Inizializzazione l'indice di Moran, tramite richiesta al backend*/
  const initializeMoran = async () => {
  ...
  return {resetAll, initializeHouse, initializeHouseBici, initializeSuggestArea, initializedValutazioneZone, checkQuestionario};
};
